...
def val(res): # True : 설치 가능, False : 설치/해제 불가능
    for x, y, s in res: 
        if s == 0:  # 기둥이 성립하기 위한 3가지 조건 (s == 0)
            # 기둥은 바닥 위에 있거나 2) 보의 한쪽 끝 부분 위에 있거나 3) 다른 기둥 위에 있어야 합니다. 4) x가 5를 넘으면 안된다.
            if y == 0 or (x-1,y,1) in res or (x-1,y,0) in res or (x,y-1,1) in res:
                return True 
            
        else: # 보가 설립하기 위한 2가지 조건 (S== 1)
            # 보는 한쪽 끝 부분이 기둥 위에 있거나, 양쪽 끝 부분이 다른 보와 동시에 연결되어 있어야 합니다.
            if ((x,y-1,0)) in res or ((x-1,y,1) in res and (x+1,y,1) in res):
                return True 
